                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data['status']) {
                            ...
                            $('<div class="col-xs-12 col-sm-12 w3-center">').html("<div class='post_item post_item_"+post_id+" col-xs-8 col-xs-offset-2 col-sm-8 col-sm-offset-2' onmouseover='showDeleteButton("+post_id+")' onmouseout='hideDeleteButton("+post_id+")'><div class='col-xs-3 col-sm-3 w3-left'><a href='users/"+user_post_id+"' style='font-size:15px'><img style='border-radius: 30px;' width='60' height='60' src='"+"{{Auth::user()->photo ? $path.Auth::user()->photo->path :$path.'/images/noimage.png'}}"+"'/></a></div><div class='col-xs-6 col-sm-6 w3-left post_info'><a href='users/"+user_id_post+"' style='font-size:15px'>{{Auth::user()->name}}</a><br><span>"+time+"</span></div>"+delete_post_button+"<hr><hr><p id='post_item_text_"+post_id+"'>"+text+"</p><hr>"+post_image_block+like_block+"<span onclick='showComments("+post_id+")' style='float:left;'>Comments</span>"+comment_block+"</div></div>").prependTo('#wall_posts');
                            new Noty({
                                type: 'success',
                                layout: 'bottomLeft',
